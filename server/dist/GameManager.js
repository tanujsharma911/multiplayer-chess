import { ERROR, GAME_OVER, INIT_GAME, INQUEUE, MOVE, EXIT_GAME, TIME_OUT, RESIGN, CHAT, PLAYER_LEFT, GET_ME, } from './messages.js';
...
export class GameManager {
    ...
    removeUser(user) {
        ...
        const game = this.games.find((g) => g.player1.userId === user.userId || g.player2.userId === user.userId);
        if (game) {
    ...
    addHandler(user) {
        user.socket.on('message', (msg) => {
            ...
            // --------------------------------- MOVE ----------------------------------
                ...
                const game = this.games.find((board) => board.player1.userId === user.userId ||
